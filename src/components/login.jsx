...
const Login = () => {
  const handleSuccess = async (googleRes) => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:3001/api/auth/google", { googletoken: idToken }, {withCredentials: true});
      
      console.log(res.data);
      //output:- {message="SUCCESS", user={name, email, photo}, exist=false}
      // if exist=true: output:- {message="SUCCESS", user, exist=true, mentee/mentorData}
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-100 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl p-8 w-full max-w-md">
        ...
        <div className="mt-6 text-center text-sm text-gray-500 space-y-2">
          ...
          <p>
            <Link to="/choice" className="text-blue-600 hover:underline">
              New user? Register
            </Link>
          </p>
        </div>
